...
function handleExitButton() {
  ...
  exitButton.onclick = async () => {
    ...
    for (let i = 0; i < commands.length; i++) {
      processMessage.innerHTML = `
        <div style=\"display:flex; flex-direction:column; align-items:center; justify-content:center;\">
          ...
          <div style=\"font-size:2.2rem; font-weight:bold; color:#23262f;\">${
            commands[i].msg
          }</div>
        </div>
      `;
      document.querySelector(".process-box").style.background =
        processBgColors[i + 1];

      await writer.write(commands[i].cmd);
    ...
    await writer.write("X"); // 시리얼로 'X' 전송
...
async function startProcess() {
  ...
  const openOrStopped = await waitForAnyArduinoResponse(
    [
    ...
    const loop = async () => {
      try {
        ...
        if (value) {
          ...
          const hitIdx = normalizedTargets.findIndex((t) =>
    ...
    const loop = async () => {
      try {
        ...
        if (value) {
          ...
          if (
            receivedData.includes("HAND DETECTED!") ||
            receivedData.includes("23")
          ) {
            processMessage.innerHTML = `
              <div style=\"display:flex; flex-direction:column; align-items:center; justify-content:center;\">
                <div style=\"margin-bottom:24px;\">
...
async function motorStop() {
  if (writer) {
    try {
      await writer.write("X"); // 모터 정지 신호 전송
    ...
    console.error(
      "Writer가 초기화되지 않았습니다. 모터 정지 신호를 보낼 수 없습니다."
    );
  }
}
